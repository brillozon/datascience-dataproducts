...
shinyUI(fluidPage(
  ...
  sidebarLayout(position="right",
    ...
    mainPanel(plotOutput("chart"),
              ...
              p("Security data is obtained from Yahoo Finance, so only those securities available from them can be viewed and returns calcualted.  The calendar range selections are adjusted to allow only dates for which data was provided to be selected.  Only daily OHLC (Open, High, Low, Close) price data is displayed, with the Closing price used for return calculations.")
              )
  )
))
